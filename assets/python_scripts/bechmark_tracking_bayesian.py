...
def calculate_crediable_region(
    ...
    while cummulated_integral < crediable_threshold:
        ...
        y_min = min(y, y_min)
        y_max = max(y, y_max)
    return crediable_region, x_min, x_max, y_min, y_max, x_likely, y_likely
...
observed_values = [-0.28, -0.28, -0.52, -0.42, -0.19, -0.20, -0.40, 0.34, -0.11, -0.82]
grid_size = 400
mu_space = 3
sigma_space = 5
...
cr, x_min, x_max, y_min, y_max, x_likely, y_likely = calculate_crediable_region(
    posterior, 0.95
)
...
print("sigma, mu, max_likelihood:", sigma_values[y_likely], mu_values[x_likely])
...
fig.colorbar(cax)
plt.savefig("DKIGI_bayesian.png", dpi=400)


observed_values = [-0.54, 1.06, 1.01, -1.17, 0.07]
prior = np.ones((grid_size, grid_size))
mu_values = np.linspace(-mu_space, mu_space, grid_size)
sigma_values = np.linspace(0.001, sigma_space, grid_size)
posterior = calculate_posterior(observed_values[:5], mu_values, sigma_values, prior)
...
fig.colorbar(cax)
plt.savefig("SPVIGAKL_bayesian.png", dpi=400)


observed_values = [-0.94, -1.34, 2.37, -0.07, 0.16, 0.96, -2.02, 0.27, 2.63]
prior = np.ones((grid_size, grid_size))
mu_values = np.linspace(-mu_space, mu_space, grid_size)
sigma_values = np.linspace(0.001, sigma_space, grid_size)
posterior = calculate_posterior(observed_values[:5], mu_values, sigma_values, prior)
...
fig.colorbar(cax)
plt.savefig("SPIEMIKL_bayesian.png", dpi=400)
